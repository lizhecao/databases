...
function obj:start()
  ...
  self.window_filter:subscribe({
    ...
    -- print("focus", window:title())
    -- print("windows", hs.window.focusedWindow())
    -- print("app", hs.application.frontmostApplication():name())
...
function obj:updateMouse(window)
  ...
  if not current_pos:inside(frame) then
    hs.mouse.absolutePosition(winframe.center)
  end
end
